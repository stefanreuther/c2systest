...
test 'host/19_kick_join', sub {
    ...
    # Configure the game: join player 1
    ...
    wait_for_turn($hc, 2);
    assert_equals conn_call($dbc, qw(hget game:1:turn:2:info turnstatus)), "\0" x 22;
    ...
    # Run one more turn. Player 1 has not submitted a turn for three turns now and will be kicked.
    conn_call($hc, qw(gameset 1 hostRunNow 1));
    wait_for_turn($hc, 4);
    ...
    # Final turn. Player 2 will be kicked, too.
    conn_call($hc, qw(gameset 1 hostRunNow 1));
    wait_for_turn($hc, 5);
    ...
    assert_contains $history[0], "game-kick:1:$user2:2";
    assert_contains $history[1], "game-kick:1:$user1:1";
    assert_contains $history[2], "game-join-other:1:$user2:2";
    assert_contains $history[3], "game-state:1:running";
